...
OVERLAP_MIN_DARK_PIXELS = 20
OVERLAP_MARGIN_POINTS = 3
BENEFICIARY_COLUMNS = (
    "Name",
    "ID",
    "DOB",
...
def _validate_field_specs(fields: list[ParsedField], *, input_pdf: str | Path | None) -> list[ValidationIssue]:
    ...
    if input_pdf is not None:
        ...
        issues.extend(_validate_original_content_overlap(input_pdf, fields))
...
def _validate_original_content_overlap(input_pdf: str | Path, fields: list[ParsedField]) -> list[ValidationIssue]:
    if shutil.which("pdftoppm") is None:
        return [
            ValidationIssue(
                "WARN",
                "original-content-check-skipped",
                "Could not run overlap check because pdftoppm is not installed or not in PATH.",
            )
        ]
    ...
    try:
        ...
        return [
            ValidationIssue(
                "WARN",
                "original-content-check-skipped",
                "Could not run overlap check because Pillow is not installed.",
            )
        ]

    issues: list[ValidationIssue] = []
    fields_by_page: dict[int, list[ParsedField]] = defaultdict(list)
    for field in fields:
        if field.field_type in {"text", "textarea", "image"}:
            fields_by_page[field.page].append(field)

    if not fields_by_page:
        return issues

    with pikepdf.Pdf.open(str(input_pdf)) as pdf, tempfile.TemporaryDirectory() as temp_dir:
        page_sizes: dict[int, tuple[float, float]] = {}
        for page_number, page in enumerate(pdf.pages, start=1):
            media_box = [float(value) for value in page.obj.get("/MediaBox", [0, 0, 612, 792])]
            page_sizes[page_number] = (media_box[2] - media_box[0], media_box[3] - media_box[1])

        for page_number, page_fields in sorted(fields_by_page.items()):
            if page_number not in page_sizes:
                continue

            prefix = Path(temp_dir) / f"page_{page_number}"
            command = [
                "pdftoppm",
                "-f",
                str(page_number),
            ...
            try:
                ...
                issues.append(
                    ValidationIssue(
                        "WARN",
                        "original-content-check-skipped",
                        f"Could not render page {page_number} for overlap check: {exc}",
                        page=page_number,
                    )
                )
                continue

            images = sorted(Path(temp_dir).glob(f"page_{page_number}-*.png"))
            if not images:
                issues.append(
                    ValidationIssue(
                        "WARN",
                        "original-content-check-skipped",
                        f"pdftoppm did not produce an image for page {page_number}.",
                        page=page_number,
                    )
                )
                continue
            ...
            page_w, page_h = page_sizes[page_number]
            scale_x = image.width / page_w
            scale_y = image.height / page_h

            for field in page_fields:
                dark_pixels, total_pixels, ratio = _measure_dark_pixels(image, field, page_h, scale_x, scale_y)
                if total_pixels == 0:
                    continue
                if dark_pixels >= OVERLAP_MIN_DARK_PIXELS and ratio >= OVERLAP_DARK_RATIO_LIMIT:
                    issues.append(
                        ValidationIssue(
                            "WARN",
                            "field-over-original-content",
                            f"Field area contains existing dark content ({ratio:.1%} dark pixels). Check that it does not cover original text.",
                            page=field.page,
                            field=field.name,
                        )
                    )
...
def _measure_dark_pixels(image: object, field: ParsedField, page_h: float, scale_x: float, scale_y: float) -> tuple[int, int, float]:
    margin = min(OVERLAP_MARGIN_POINTS, field.w / 4, field.h / 4)
    left = int(round((field.x + margin) * scale_x))
    right = int(round((field.x + field.w - margin) * scale_x))
    top = int(round((page_h - field.y - field.h + margin) * scale_y))
    bottom = int(round((page_h - field.y - margin) * scale_y))
    ...
    dark = sum(1 for value in pixels if value < OVERLAP_DARK_PIXEL_THRESHOLD)
    return dark, total, dark / total
