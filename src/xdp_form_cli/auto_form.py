...
# Maximum seconds allowed for any subprocess call (pdftotext, pdftoppm, etc.).
...
def write_field_csv(specs: list[AutoFieldSpec], csv_path: str | Path, *, overwrite: bool = False) -> Path:
    path = Path(csv_path)
    from xdp_form_cli.safe_io import backup_if_exists, require_safe_output
    require_safe_output(path, overwrite=overwrite)
    if overwrite:
...
def write_mapping_report(specs: list[AutoFieldSpec], report_path: str | Path, *, overwrite: bool = False) -> Path:
